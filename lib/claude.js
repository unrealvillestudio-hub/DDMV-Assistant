...
function getCaregiverPrompt(userName, botName, damarisName) {
  const nowPA = new Date(Date.now() - 5 * 60 * 60 * 1000);
  const dateStr = nowPA.toLocaleDateString('es-PA', { weekday:'long', year:'numeric', month:'long', day:'numeric' });
  const timeStr = nowPA.toLocaleTimeString('es-PA', { hour:'2-digit', minute:'2-digit' });
...
TONO: Directo y profesional con ${userName}. Él conoce el sistema.`;
}

export async function chat(messages, userName, botName, role = 'user', damarisName = 'Damaris') {
  const system = role === 'caregiver'
    ? getCaregiverPrompt(userName, botName, damarisName)
